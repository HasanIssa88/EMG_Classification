...
def EMG_Threshold(inFile,threshold = 0.3):
    ...
    df = pd.read_csv(inFile,sep=',',skiprows=1,index_col=0)
    ...
    ax3.set_ylim(0,0.1)
    ax3.set_yticklabels([])
    ax3.set_ylabel('Ch1[mV]')   
    
    ax4.plot(time,EMG_radial_3)
    ax4.set_ylim(0,0.1)
    ax4.set_yticklabels([])
    ax4.set_ylabel('Ch1[mV]') 
    
    ax5.plot(time,EMG_radial_4)
    ax5.set_ylim(0,0.1)
    ax5.set_yticklabels([])
    ax5.set_ylabel('Ch1[mV]')  
    
    ax6.plot(time,EMG_radial_5)
    ax6.set_ylim(0,0.1)
    ax6.set_yticklabels([])
    ax6.set_ylabel('Ch1[mV]')    
    
    
    ax7.plot(time,EMG_radial_6)
    ax7.set_ylim(0,0.1)
    ax7.set_yticklabels([])
    ax7.set_ylabel('Ch1[mV]')    
    ...
    print(df.head())
    print(df.shape)
    df.to_csv('EMG_Threshold.txt', sep =',',index=None)
    
    return(df)
    
EMG_Threshold('06_01_precision_10_010.txt')